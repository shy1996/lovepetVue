...
//更新新闻数据成功
router.post('/updateNewAction', upload.single('newUrl'),function (req, res, next) {
    var obj = req.body;
    ...
    fs.rename("upload/"+filename, "upload/"+originalname, (err) => {
        ...
        mysql.connect((db) => {
            var updateObj = {
                newID: obj.newID,
                newUrl: "http://localhost:3000/"+originalname,
...
//添加新闻信息保存数据
router.post('/addNewAction', upload.single('newUrl'), function (req, res, next) {
       var obj = req.body;
       var filename = req.file.filename;
       var originalname = req.file.originalname;
       var pageCode = url.parse(req.url, true).query.pageCode;
